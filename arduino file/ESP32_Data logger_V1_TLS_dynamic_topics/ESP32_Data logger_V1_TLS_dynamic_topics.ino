...
// -----------------------------------------------------------------
// MQTT ALIGNMENT (Self-hosted EMQX/Mosquitto compatible)
// Fill these when you have your broker ready. Safe defaults left in.
// NOTE: Only MQTT section is aligned; no other logic changed.
// -----------------------------------------------------------------

// For A7672S (Jio): use APN "jionet" on your GSM stack when you wire it in.
// const char* GSM_APN = "jionet"; // <-- uncomment/use in your GSM code

// --- Required placeholders you asked for ---
// BROKER = <your broker IP/domain>
// AUTH   = <no auth / or username+password>
// REALTIME = YES

// If you keep no auth, leave MQTT_USERNAME/MQTT_PASSWORD empty.
const char* MQTT_USERNAME = "";   // e.g., "emqx_user" or "" for no auth
const char* MQTT_PASSWORD = "";   // e.g., "strongpass" or "" for no auth

// Device identity (adjust per device to support 10+ units cleanly)
#define DEVICE_ID   "ESP_logger_1"   // e.g., ESP_logger_1, ESP_logger_2, ...
#define ASSET_CODE  "AP_1001"        // e.g., AP_1001, AP_500, ...

// Topic scheme you requested:
//   /devices/<DEVICE_ID>/<ASSET_CODE>/realtime
//   Example: /devices/ESP_logger_1/AP_1001/realtime
#define TOPIC_REALTIME  "/devices/" DEVICE_ID "/" ASSET_CODE "/realtime"

// Optional: where to publish notifications/metadata about finished 50MB files
#define TOPIC_FILES     "/devices/" DEVICE_ID "/" ASSET_CODE "/files"

// 60-second publish cadence for telemetry
const unsigned long MQTT_PUBLISH_INTERVAL_MS = 60000;

// For TLS later: change MQTT_PORT to 8883 and use a secure client & CA certs.
// PubSubClient works with WiFiClientSecure/TinyGsmClientSecure when you wire it.
// -----------------------------------------------------------------

// Mock Network Client for MQTT (Global declaration for scope)
// NOTE: For MQTT over TLS (port 8883), replace Client with a secure client
// e.g., WiFiClientSecure or TinyGsmClientSecure and load CA certs.
Client gsmClient; 
...
const char* PREFS_KEY_DEVICE_ID = "device_id";
const char* PREFS_KEY_ASSET_CODE = "asset_code";
...
const char* DEFAULT_DEVICE_ID = "ESP_logger_1";
const char* DEFAULT_ASSET_CODE = "AP_1001";
...
struct MachineConfig {
    ...
    char deviceId[32];          // MQTT Device ID
    char assetCode[32];         // MQTT Asset Code
};
...
void loadConfiguration() {
    ...
    // Load new MQTT identification fields
    preferences.getString(PREFS_KEY_DEVICE_ID, DEFAULT_DEVICE_ID, currentConfig.deviceId, sizeof(currentConfig.deviceId));
    preferences.getString(PREFS_KEY_ASSET_CODE, DEFAULT_ASSET_CODE, currentConfig.assetCode, sizeof(currentConfig.assetCode));
...
void saveConfiguration() {
    ...
    // Save MQTT identification fields
    preferences.putString(PREFS_KEY_DEVICE_ID, currentConfig.deviceId);
    preferences.putString(PREFS_KEY_ASSET_CODE, currentConfig.assetCode);
...
void enter_operational_mode() {
    ...
    // --- MQTT client endpoint (self-host) ---
    // Update MQTT_SERVER/MQTT_PORT above. For TLS use port 8883 with a secure client.
    mqttClient.setServer(MQTT_SERVER, MQTT_PORT);
    // Optional auth when you enable it on the broker:
    // Use MQTT_USERNAME/MQTT_PASSWORD in your connect logic.
...
<div class="container mx-auto p-6 bg-white rounded-xl shadow-2xl border-t-4 border-indigo-500">
    ...
    if (isLoggedIn) {
        ...
        html += R"raw(" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-3 border focus:ring-indigo-500 focus:border-indigo-500 transition duration-150">
                ...
                <div>
                    <label for="deviceId" class="block text-sm font-medium text-gray-700">Device ID (for MQTT topic)</label>
                    <input type="text" id="deviceId" name="deviceId" required maxlength="31"
                           value=")raw";
        html += String(currentConfig.deviceId);
        html += R"raw(" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-3 border focus:ring-indigo-500 focus:border-indigo-500 transition duration-150">
                </div>
                <div>
                    <label for="assetCode" class="block text-sm font-medium text-gray-700">Asset Code (for MQTT topic)</label>
                    <input type="text" id="assetCode" name="assetCode" required maxlength="31"
                           value=")raw";
        html += String(currentConfig.assetCode);
        html += R"raw(" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-3 border focus:ring-indigo-500 focus:border-indigo-500 transition duration-150">
                </div>
            </div>
...
void handleSave() {
    ...
    if (server.hasArg("deviceId")) {
        strncpy(currentConfig.deviceId, server.arg("deviceId").c_str(), sizeof(currentConfig.deviceId) - 1);
        currentConfig.deviceId[sizeof(currentConfig.deviceId) - 1] = '\0';
    }
    if (server.hasArg("assetCode")) {
        strncpy(currentConfig.assetCode, server.arg("assetCode").c_str(), sizeof(currentConfig.assetCode) - 1);
        currentConfig.assetCode[sizeof(currentConfig.assetCode) - 1] = '\0';
    }
...
void mqtt_sender_task(void *pvParameters) {
    ...
    while (currentMode == OPERATIONAL_MODE) {
        if (millis() - lastMqttSendTime >= MQTT_PUBLISH_INTERVAL_MS) { // Check and upload every 60s
            // MOCK GSM Network Check & MQTT Connect
            Serial.println("MOCK GSM/MQTT: Uploading data...");
            // Topic per your scheme (dynamic from config)
            String topicRealtime = String("/devices/") + String(currentConfig.deviceId) + "/" + String(currentConfig.assetCode) + "/realtime";
            // Example publish when MQTT is wired in:
            // if (mqttClient.connected()) { mqttClient.publish(topicRealtime.c_str(), msg.c_str()); }
            Serial.print("Topic: "); Serial.println(topicRealtime);
